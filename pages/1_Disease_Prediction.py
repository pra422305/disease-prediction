...
display_names   = ["— Select —"] + sorted(symptom_display.keys())
...
with col1: s1 = st.selectbox("Symptom 1 *", display_names, key="s1")
with col2: s2 = st.selectbox("Symptom 2 *", display_names, key="s2")
with col3: s3 = st.selectbox("Symptom 3",   display_names, key="s3")
with col4: s4 = st.selectbox("Symptom 4",   display_names, key="s4")
with col5: s5 = st.selectbox("Symptom 5",   display_names, key="s5")

with st.expander("➕ Add more symptoms (advanced)", expanded=False):
    extra = st.multiselect("Additional symptoms", options=sorted(symptom_display.keys()))
...
if predict_btn:
    ...
    chosen = list(dict.fromkeys(
        symptom_display[d] for d in raw_selected if d != "— Select —"
    ))
    ...
    if len(chosen) < 2:
        ...
        if "error" in result:
            ...
            chips = " &nbsp; ".join(
                f"<span class='badge badge-medium'>{pretty(s)}</span>" for s in chosen
            )
            ...
            with gc:
                st.plotly_chart(plotly_gauge(confidence, disease), use_container_width=True)
            with pc:
                st.plotly_chart(plotly_confidence_chart(top5), use_container_width=True)
            ...
            if info:
                ...
                with dc2:
                    ...
                    <div class='info-card'><h4>🩺 Common Symptoms</h4><p>{info.get('symptoms','—')}</p></div>
                    """, unsafe_allow_html=True)

            st.markdown("""
            <div class='warning-box' style='margin-top:1.5rem'>
            ⚠️ <strong>Medical Disclaimer:</strong> This prediction is for educational purposes only.
            Always consult a licensed healthcare provider.
            </div>
            """, unsafe_allow_html=True)

st.markdown("<div class='footer'>MedAI 🏥 · For educational use only · Always consult a doctor</div>", unsafe_allow_html=True)
